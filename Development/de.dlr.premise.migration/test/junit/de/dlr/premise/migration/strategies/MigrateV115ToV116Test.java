 ...
 *
 */
public class MigrateV115ToV116Test {

    private static String NAME = "MigrateV115ToV116";

    private static String FILENAME_PREMISE = "MigrateV115ToV116.premise";
    ...
    public void testMigration() throws IOException, SAXException, ParserConfigurationException, MigrationMissingException {
        ...
        MigrationStrategy.migrate(model, "1.16");
        model.save();

        assertTrue(model.wasChanged());
        assertEquals("1.16", model.getModelRoot().getAttribute("metaModel"));

        assertEquals(971, new File(PATH_OUTPUT).length());
