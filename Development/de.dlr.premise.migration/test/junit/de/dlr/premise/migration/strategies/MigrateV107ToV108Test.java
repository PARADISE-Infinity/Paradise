...
public class MigrateV107ToV108Test {
	private static String NAME = "MigrateV107ToV108";
	private static String FILENAME_PREMISE = "MigrateV107ToV108.premise";
	...
	public void testMigration() throws IOException, SAXException, ParserConfigurationException, MigrationMissingException {
		...
		MigrationStrategy.migrate(model, "1.08");
		model.save();

		assertTrue(model.wasChanged());
		assertEquals("1.08", model.getModelRoot().getAttribute("metaModel"));

		assertEquals(970, new File(PATH_OUTPUT).length());
