...
public class MigrateV106ToV107Test {
	private static String NAME = "MigrateV106ToV107";
	private static String FILENAME_PREMISE = "MigrateV106ToV107.premise";
	...
	public void testMigration() throws IOException, SAXException, ParserConfigurationException, MigrationMissingException {
		...
		MigrationStrategy.migrate(model, "1.07");
		model.save();

		assertTrue(model.wasChanged());
		assertEquals("1.07", model.getModelRoot().getAttribute("metaModel"));

		assertEquals(1271, new File(PATH_OUTPUT).length());
